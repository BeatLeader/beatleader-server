        ...
        public async Task<ActionResult> newranking()
        {
                    ...
                    if (diff.Status == DifficultyStatus.ranked || diff.Status == DifficultyStatus.qualified || diff.Status == DifficultyStatus.nominated) {
                        ...
                        if (response != null)
                        {
                            ...
                            diff.ModifiersRating = new ModifiersRating
                            {
                                ...
                                FSPassRating = RankController.ModifiersBuff(response.FS.lack_map_calculation.balanced_pass_diff, "FS"),
                                FSTechRating = response.FS.lack_map_calculation.balanced_tech * 10,
                                FSPredictedAcc = response.FS.AIacc,
                                SFPassRating = RankController.ModifiersBuff(response.SFS.lack_map_calculation.balanced_pass_diff, "SF"),
            ...
            foreach (var leaderboard  in leaderboards) {
                ...
                LeaderboardChange rankChange = new LeaderboardChange
                {
                    ...
                    NewStars = ReplayUtils.ToStars(diff.AccRating ?? 0, diff.PassRating ?? 0, diff.TechRating ?? 0, diff.PatternRating ?? 0, diff.LinearRating ?? 0, diff.PredictedAcc ?? 0, diff.ModeName),
